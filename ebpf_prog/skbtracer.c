...
do_trace_skb(struct event_t *event, void *ctx, struct sk_buff *skb, void *netdev) {
    ...
    if (filter_pid() || filter_netns(skb, netdev) || filter_l3_and_l4_info(skb))
        return false;

    event->flags |= SKBTRACER_EVENT_IF;
    set_event_info(skb, ctx, event);
    set_pkt_info(skb, &event->pkt_info, netdev);
    set_ether_info(skb, &event->l2_info);

    switch (event->l2_info.l3_proto) {
    case ETH_P_IP: set_ipv4_info(skb, &event->l3_info); break;
    case ETH_P_IPV6: set_ipv6_info(skb, &event->l3_info); break;
    ...
    case IPPROTO_TCP: set_tcp_info(skb, &event->l4_info); break;
    case IPPROTO_UDP: set_udp_info(skb, &event->l4_info); break;
    case IPPROTO_ICMP:
    case IPPROTO_ICMPV6:
        set_icmp_info(skb, &event->icmp_info); break;
    default:
        ret = false; break;
    }
...
do_trace(void *ctx, struct sk_buff *skb, const char *func_name, void *netdev)
{
    ...
    if (!do_trace_skb(event, ctx, skb, netdev))
        return 0;
...
__ipt_do_table_out(struct pt_regs *ctx, struct sk_buff *skb)
{
    ...
    if (!do_trace_skb(event, ctx, args->skb, NULL))
        return 0;
